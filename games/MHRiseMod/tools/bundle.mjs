/**
 * Pack sibling RefShell + the MH Rise menu into one autorun script.
 *
 * Usage: npm run bundle
 *        node tools/bundle.mjs <out.lua>
 * Default output: dist/cache/mhrise_qol.lua
 *
...
const OUT = path.resolve(process.argv[2] || path.join(ROOT, "dist", "cache", "mhrise_qol.lua"));
...
const LOCAL_MODULES = [
  ...
  ["freetree", "reframework/autorun/freetree.lua"],
];
...
function main() {
  ...
  const bundled = `--[[
  mhrise_qol.lua — generated release bundle. Do not edit.

  Build: npm run bundle
  Install as: reframework/autorun/mhrise_qol.lua
...
${parts.join("\n")}`;
  ...
  if (!bundled.includes("MH RISE QOL")) {
    throw new Error("Bundle must include the MH Rise menu");
  }
  ...
  if (!bundled.includes("getHpVital()")) {
    throw new Error("Bundle must bind EnemyCharacterBase.getHpVital");
  }
  if (!bundled.includes("Health Bars")) {
    throw new Error("Bundle must include the Health Bars menu");
  }
  if (!bundled.includes('package.preload["freetree"]')) {
    throw new Error("Bundle must preload freetree");
  }
  if (!bundled.includes("MHFreeTree")) {
    throw new Error("Bundle must assign _G.MHFreeTree");
  }
  if (!bundled.includes("write_dword")) {
    throw new Error("Bundle must zero recipe arrays via write_dword");
  }
